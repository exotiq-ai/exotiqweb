...
export function useFormSubmission() {
  ...
  const submitForm = async (formData: FormData, formType: 'beta' | 'contact') => {
    ...
    try {
      ...
      // Report the conversion to GA4 / Meta / PostHog. This runs only after the
      // server confirmed success, so a conversion is never reported for a
      // failed submission. No e-mail or other personal data is passed on.
      if (formType === 'beta') {
